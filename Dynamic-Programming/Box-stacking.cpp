...
void solve(){
    ...
    vector<int> b(n);
    for(int i=0;i<n;i++) b[i]=v[i].second.second;
    for(int i=1;i<n;i++){
        for(int j=0;j<i;j++){
            if(v[i].first>v[j].first and v[i].second.first>v[j].second.first and v[i].second.second>v[j].second.second and b[i]<b[j]+v[i].second.second)
                b[i]=b[j]+v[i].second.second;
        }
    }
    cout<<*max_element(range(b))<<endl;
}
