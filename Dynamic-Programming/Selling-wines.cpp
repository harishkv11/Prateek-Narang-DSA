...
//---------------------------------
int solve(vector<int> p,int i,int j,int year,vector<vector<int>> &dp){
    if(i>j) return 0;
    if(i==j) return year*p[i];
    else if(dp[i][j]!=-1) return dp[i][j];
    else{
        int op1=solve(p,i+1,j,year+1,dp)+p[i]*year;
        int op2=solve(p,i,j-1,year+1,dp)+p[j]*year;
        dp[i][j]=max(op1,op2);
        return dp[i][j];
    }
}
