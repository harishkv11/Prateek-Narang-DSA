...
//Top-Down 
int solverec(int n,int k,vector<int> &dp){
    if(!n) return 1;
    if(dp[n]!=-1) return dp[n];
    int ans=0;
    for(int i=1;i<=k;i++){
        if(n-i>=0) ans+=solverec(n-i,k,dp);
    }
    return dp[n]=ans;
}

//Bottom-Up 
int solveiter(int n,int k){
    vector<int> dp(n+1);
    dp[0]=1;
    for(int j=1;j<=n;j++){
        for(int i=1;i<=k;i++){
...
void solve(){
    int n,k;cin>>n>>k;
    vector<int> dp(n+1,-1);
    cout<<solverec(n,k,dp)<<endl;   
    cout<<solveiter(n,k)<<endl;
    cout<<solveoptim(n,k)<<endl;
}
